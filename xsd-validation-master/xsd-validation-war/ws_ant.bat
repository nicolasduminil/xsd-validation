...
@REM check for existing profile.
set CMD_NAME=%~nx0
set PROFILE_ERROR=0
...
set "CLASSPATH=%WAS_HOME%\plugins\com.ibm.ws.prereq.ow.asm.jar;%WAS_CLASSPATH%"

java -Djava.endorsed.dirs="%WAS_ENDORSED_DIRS%" -Dcmd.properties.file=%TMPJAVAPROPFILE% %WAS_LOGGING% %CONSOLE_ENCODING% "%CLIENTSAS%" "%CLIENTSSL%" "-DWAS_USER_SCRIPT=%WAS_USER_SCRIPT%" %USER_INSTALL_PROP% -Dwas.install.root="%WAS_HOME%" "-Dwas.root=%WAS_HOME%" com.ibm.ws.bootstrap.WSLauncher org.apache.tools.ant.Main %*
set RC=%ERRORLEVEL%
